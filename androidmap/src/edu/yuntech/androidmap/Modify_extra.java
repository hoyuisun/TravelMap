...
public class Modify_extra extends Activity implements ConnectionCallbacks, OnConnectionFailedListener, LocationListener{
	
	private String uriAPI = "http://140.125.45.113/contest/post_mysql/modify_extra.php";
	/** 「要更新版面」的訊息代碼 */
	...
	Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case REFRESH_DATA:
				...
				if (result != null)
					// 印出網路回傳的文字
					Toast.makeText(Modify_extra.this, "上傳成功", Toast.LENGTH_LONG).show();
				break;
			}
		}
	};
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.modify_extra);
        ...
        submit.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				Thread t = new Thread(new sendPostRunnable(data.getString("select"), data.getString("id"), name_tw.getText().toString(), site_tw.getText().toString(), 
						context.getText().toString(), lat.getText().toString(), lng.getText().toString()));
				t.start();
				finish();
			}
        });
	...
	class sendPostRunnable implements Runnable {
		String select, id, name_tw, context, site_tw, lat, lng;
		// 建構子，設定要傳的字串
		public sendPostRunnable(String select, String id, String name_tw, String site_tw, String context, String lat, String lng) {
			this.select = select;
			this.id = id;
		...
		@Override
		public void run() {
			String result = sendPostDataToInternet(select, id, name_tw, site_tw, context, lat, lng);
			mHandler.obtainMessage(REFRESH_DATA, result).sendToTarget();
		}

	}

	private String sendPostDataToInternet(String select, String id, String name_tw, String site_tw, String context, String lat, String lng) {
		...
		params.add(new BasicNameValuePair("phone", context));
